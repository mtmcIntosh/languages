...
x=0
y=0
xstep=[0, 2, 3, 5, 7 ,8, 10, 12, 13, 15]
ystep=[0, 2, 3, 5, 7, 8, 10, 12, 13, 15]
for atom in xrange(Natoms):
    x_positions[atom,0]=xstep[x]*R_min_energy
    y_positions[atom,0]=ystep[y]*R_min_energy    
    if x < atoms_in_X-1:
        x=x+1
    else:
        x=0
    ...
    VAF_atoms[atom] = x_velocities_t0[atom]**2+y_velocities_t0[atom]**2
...
outputfile=open(outputPOSfilename,"wb")
data_file=csv.writer(outputfile, delimiter=",")   
