"""KMCS to model co-adsorption
of A and B on a square lattice. 
...
k_indicator = '_allsamek__A5_EA5E3_1'
KMCSdescription = 'AtoBcoadsorb'
...
total_time = 10         #s
#environment ----------------------
T_array = np.linspace(101,401, 4)     #K
...
class particle(object):
    ...
    def __init__(self, x, y, T):
        ...
        self.k_B_init = A_B * math.exp(-E_B/Kb/ self.T)
        ...
        self.k_B = self.k_B_init
        self.k_Bx = 0
        self.k_Ax = 0

    def adsorb (self, *args):
    ...
    def events (self, events_total):
        ...
        self.S_kB = self.k_B / events_total * 100
...
for T in T_array:
    ...
    while time <= total_time:
        ...
        events_total=0
        for atom in atoms:
            events_total = events_total + atom.k_A + atom.k_Ax + atom.k_B + atom.k_Bx
        if events_total ==0:
            break;
        ...
        for atom in atoms:
            if ((step) < a <= (step + atom.S_kA)) and atom.S_kA != 0:
                ...
                x_position.append(atom.x_position)
                y_position.append(atom.y_position)
            elif (step + atom.S_kA + atom.S_kAx)  < a <= (step + atom.S_kA + atom.S_kAx + atom.S_kB) and atom.S_kB != 0:
                processes.append('k_B')
                processID.append(2)
                atom.adsorb('B')
                x_position.append(atom.x_position)
                y_position.append(atom.y_position)
            elif (step + atom.S_kA + atom.S_kAx + atom.S_kB)  < a <= \
                 (step + atom.S_kA + atom.S_kAx + atom.S_kB + atom.S_kBx) and atom.S_kBx != 0:
                processes.append('k_Bx')
                processID.append(3)
                atom.desorb('B')
                x_position.append(atom.x_position)
                y_position.append(atom.y_position)
            step = (step + atom.S_kA + atom.S_kAx + atom.S_kB + atom.S_kBx)
    ...
    plt.title('A and B adsorption ',fontsize=12)
    plt.ylabel('number',fontsize=12)
    ax2.set_xticklabels([])
    plt.xlim([0, total_time])
