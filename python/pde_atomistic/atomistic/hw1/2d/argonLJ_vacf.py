...
outputVACFfilename = "./Ar_LJ_VACF.csv"
...
total_time = 2*ps_to_s         #s
time = 0.0*ps_to_s              #s
time_in_step = np.arange(time,total_time,timestep,dtype=float)
tcorr={}
t0=time
for t in xrange(len(time_in_step)):
    
    while t0 <= time_in_step[t]:
        
        tau = time_in_step[t] - t0
...
VAF_tcorr=dict.fromkeys(tcorr)
VAF_tcorr_norm=dict.fromkeys(tcorr)
...
for atom in xrange(Natoms):
    ...
    VAF_tcorr[0] = x_velocities_t0[atom]**2+y_velocities_t0[atom]**2

#initialize forces on each atom  at time_0  --------------------------------------------
(F_atoms_x, F_atoms_y, V_atoms) = \
                calcForces_and_potentialE(F_atoms_x, F_atoms_y,0, x_positions, y_positions, V_atoms)
...
for step in xrange(1, len(time_in_step)):  
    ...
    (F_atoms_x, F_atoms_y, V_atoms) = \
                calcForces_and_potentialE(F_atoms_x, F_atoms_y, 1, x_positions, y_positions, V_atoms)
    ...
    for atom in xrange(Natoms):    
            ...
            T_atoms[atom] = \
                        mass *( x_velocities[atom,step]**2 + y_velocities[atom,step]**2 )  / 2.0
            ...
            while t0 <= time_in_step[step]:
                ...
                t0index=t0index+1
                tcorrelation = time_in_step[step]-t0


                if tcorrelation in VAF_tcorr:
                    if (VAF_tcorr[tcorrelation] == None):
                        VAF_tcorr[tcorrelation] =  (x_velocities[atom,t0index-1]*x_velocities[atom,step]\
                              + y_velocities[atom,t0index-1]*y_velocities[atom,step]) / (x_velocities[atom,t0index-1]**2\
                                                  +y_velocities[atom,t0index-1]**2)
                    else:
                        VAF_tcorr[tcorrelation] +=  (x_velocities[atom,t0index-1]*x_velocities[atom,step]\
                              + y_velocities[atom,t0index-1]*y_velocities[atom,step]) / (x_velocities[atom,t0index-1]**2\
                                                  +y_velocities[atom,t0index-1]**2)
...
VAF_tcorr_norm[0]=VAF_tcorr[0]/Natoms
for key in VAF_tcorr:
    if key != 0:
        VAF_tcorr_norm[key]=VAF_tcorr[key]/(key/timestep)/Natoms/VAF_tcorr_norm[0]
VAF_tcorr_norm[0]=VAF_tcorr_norm[0]/VAF_tcorr_norm[0]

VAF_sorted = collections.OrderedDict(sorted(VAF_tcorr_norm.items()))


#write data to disk-------------------------------------------------------------------
outputfile=open(outputfilename,"wb")
data_file=csv.writer(outputfile, delimiter=",")
data_file.writerow(['Time (ps)','Total Energy (eV)','Kinetic Energy (eV)','Potential Energy (eV)'])
for row in xrange(len(time_in_step)):
    data_file.writerow([time_in_step[row]*ps_to_s**-1, E_total[row], T_total[row], V_total[row]])
outputfile.close()

posoutputfile=open(outputPOSfilename,"wb")
data_file=csv.writer(posoutputfile, delimiter=",")   
data_file.writerow(['initialpos, x', 'initialpos, y', 'finalpos, x', 'finalpos, y'])
for atom in xrange(Natoms):
    data_file.writerow([x_positions_t0[atom], y_positions_t0[atom], x_positions[atom,1], y_positions[atom,1]])
posoutputfile.close()

vacfoutputfile=open(outputVACFfilename,"wb")
data_file=csv.writer(vacfoutputfile, delimiter=",")   
data_file.writerow(['tau', 'VACF'])
for k, v in VAF_sorted.iteritems():
    data_file.writerow([k, v])
vacfoutputfile.close()
